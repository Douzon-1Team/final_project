...
public interface ProgressBar52hMapper {
    ...
    @Select("select sum(if(dayofweek(date)=7 or dayofweek(date)=1,TIMESTAMPDIFF(hour, date,off_work),TIMESTAMPDIFF(hour, date,off_work)-9)) week_worktime from attendance_time a\n" +
            "join (select empno, date as off_work from attendance_time where empno= #{empno} and on_off_work = 0) b on a.empno = b.empno\n" +
            "where on_off_work = 1 and day(a.date) = day(b.off_work) and DATE_FORMAT(date, '%Y%U') = DATE_FORMAT(now(), '%Y%U')\n" +
            "GROUP BY DATE_FORMAT(date, '%Y%U');")
