...
export const getList = createAsyncThunk("GET_TODO", async () => {
  ...
  try {
    const response = await getMain({empnos:220102});
    ...
    for (var i = 0; i < response.data.length; i++) {
      if (response.data[i].empno != null) { // 출근
        if (response.data[i].tardy == 1) { // 지각
        ...
        } else if (response.data[i].leaveEarly == true) { // 조퇴
        ...
        } else if (response.data[i].attendance == false) { // 결근
          ...
          if (response.data[i].offwork == null && response.data[i].onwork != null) {
        ...
        if (response.data[i].title == "조퇴" || response.data[i].title == "결근") continue;
        // TODO : 시간연차 추후에 추가
