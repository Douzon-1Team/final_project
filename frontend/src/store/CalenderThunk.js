  ...
  try {
    ...
    for (var i = 0; i < response.data.length; i++) {
      if (response.data[i].empno != null) {
        ...
        } else if (response.data[i].leaveEarly == 1) {
          // 조퇴
        ...
        } else if (response.data[i].attendance == 0) {
          // 결근
        ...
        _.merge(response.data[i], {
          category: "time",
          isVisible: true,
          id: i + 1,
          body: response.data[i].context,
          start: new Date(response.data[i].onwork),
          end: new Date(response.data[i].offwork),
        });

        response.data[i] = _.omit(response.data[i], [
          "accept",
          "attendance",
          "leaveEarly",
          "reject",
